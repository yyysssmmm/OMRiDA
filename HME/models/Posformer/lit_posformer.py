...
import label_make_muti
from posformer import PosFormer
from utils import (ExpRateRecorder, Hypothesis,ce_loss_all,ce_loss,
                               to_bi_tgt_out)
...
class LitPosFormer(pl.LightningModule):
    ...
    def test_step(self, batch: Batch, _):
        ...
        self.exprate_recorder([h.seq for h in hyps], batch.indices)
        self.test_outputs.append((batch.img_bases, [vocab.indices2label(h.seq) for h in hyps], inference_time))
        self.log('batch_inference_time', inference_time)  # Optional: log inference time per batch

    def on_test_epoch_end(self):

        total_inference_time = sum(output[2] for output in self.test_outputs)    # Sum up the inference times
        ...
        with zipfile.ZipFile(os.path.join(base_dir, "result.zip"), "w") as zip_f:
            for img_bases, preds, _ in self.test_outputs:
