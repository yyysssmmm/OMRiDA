...
def extract_data(archive: ZipFile, dir_name: str) -> Data:
    ...
    for line in captions:
        ...
        img_name = tmp[0].split('.')[0]
        formula = tmp[1:]

        # 우선 bmp로 시도하고, 없으면 png로 시도
        img = None
        try:
            ...
            try:
                with archive.open(f"data/{dir_name}/img/{img_name}.png", "r") as f:
                    img = Image.open(f).copy()
            except KeyError:
                print(f"[WARN] 이미지 파일 없음: {img_name}.bmp/.png")
...
class CROHMEDatamodule(pl.LightningDataModule):
    def __init__(
        self,
        zipfile_path: str = os.path.join(base_dir, "test_data_for_PosFormer.zip"),
        test_year: str = "2014",
